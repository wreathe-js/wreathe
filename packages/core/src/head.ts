...
const Renderer = {
  ...
  isWreatheManagedElement(element: Element): boolean {
    return (
      ...
      element.getAttribute('wreathe') !== null
    )
  },

  findMatchingElementIndex(element: Element, elements: Array<Element>): number {
    const key = element.getAttribute('wreathe')
    if (key !== null) {
      return elements.findIndex(
        (element) => element.getAttribute('wreathe') === key
      )
    }
...
export default function createHeadManager(
  ...
  function collect(): Array<string> {
    ...
    const defaults: Record<string, string> = {
      ...(title ? { title: `<title wreathe="">${title}</title>` } : {}),
    }
    ...
    const elements = Object.values(states)
      ...
      .reduce((carry, element) => {
        ...
        const match = element.match(/ wreathe="[^"]+"/)
        if (match) {
