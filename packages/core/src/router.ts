...
class Router {
  ...
  protected isLocationVisit(): boolean {
    try {
      return window.sessionStorage.getItem('wreatheLocationVisit') !== null
    } catch (error) {
      return false
    }
  }
  ...
  protected handleLocationVisit(page: Page): void {
    const locationVisit: LocationVisit = JSON.parse(
      window.sessionStorage.getItem('wreatheLocationVisit') || ''
    )
    window.sessionStorage.removeItem('wreatheLocationVisit')
  ...
  protected isLocationVisitResponse(response: AxiosResponse): boolean {
    return !!(
      response &&
      response.status === 409 &&
      response.headers['x-wreathe-location']
    )
  }

  protected isWreatheResponse(response: AxiosResponse): boolean {
    return !!response?.headers['x-wreathe']
  }
  ...
  public visit(
    ...
    Axios({
      ...
      headers: {
        ...
        ...(only.length
          ? {
              'X-Wreathe-Partial-Component': this.page.component,
        ...
        ...(errorBag && errorBag.length
          ...
          : {}),
        ...(this.page.version
          ? { 'X-Wreathe-Version': this.page.version }
          : {}),
      },
      ...
      .then((response) => {
        if (!this.isWreatheResponse(response)) {
      ...
      .catch((error) => {
        if (this.isWreatheResponse(error.response)) {
          ...
          const locationUrl = hrefToUrl(
            error.response.headers['x-wreathe-location']
          )
          const requestUrl = url
          if (
            requestUrl.hash &&
  ...
  public replace(
    ...
    console.warn(
      `Wreathe.replace() has been deprecated and will be removed in a future release. Please use Wreathe.${
        options.method ?? 'get'
      }() instead.`
    )
  ...
  public on<TEventName extends GlobalEventNames>(
    ...
    document.addEventListener(`wreathe:${type}`, listener)
