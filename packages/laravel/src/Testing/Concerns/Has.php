...
trait Has
{
    protected function count(string $key, int $length): self
    {
        PHPUnit::assertCount(
            $length,
            $this->prop($key),
            sprintf('Wreathe property [%s] does not have the expected size.', $this->dotPath($key))
        );
    ...
    public function has(string $key, $value = null, Closure $scope = null): self
    {
        PHPUnit::assertTrue(
            ...
            sprintf('Wreathe property [%s] does not exist.', $this->dotPath($key))
        );
    ...
    public function missing(string $key): self
    {
        ...
        PHPUnit::assertNotTrue(
            Arr::has($this->prop(), $key),
            sprintf('Wreathe property [%s] was found while it was expected to be missing.', $this->dotPath($key))
        );
