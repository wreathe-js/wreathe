    ...
    public function where(string $key, $expected): self
    {
        ...
        if ($expected instanceof Closure) {
            PHPUnit::assertTrue(
                ...
                sprintf('Wreathe property [%s] was marked as invalid using a closure.', $this->dotPath($key))
            );
        ...
        PHPUnit::assertSame(
            $expected,
            $actual,
            sprintf('Wreathe property [%s] does not match the expected value.', $this->dotPath($key))
        );
