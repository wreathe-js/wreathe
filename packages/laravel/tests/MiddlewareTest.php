...
{
    public function test_no_response_value_by_default_means_automatically_redirecting_back_for_wreathe_requests(): void
    {
        $fooCalled = false;
    ...
    public function test_no_response_value_can_be_customized_by_overriding_the_middleware_method(): void
    {
        ...
        $this->expectExceptionMessage('An empty Wreathe response was returned.');
    ...
    public function test_no_response_means_no_response_for_non_wreathe_requests(): void
    {
        $fooCalled = false;
        Route::middleware(Middleware::class)->put('/', function () use (&$fooCalled) {
            $fooCalled = true;
    ...
    public function test_it_will_instruct_wreathe_to_reload_on_a_version_mismatch(): void
    {
        $this->prepareMockEndpoint('1234');

        $response = $this->get('/', [
            'X-Wreathe' => 'true',
            'X-Wreathe-Version' => '4321',
        ]);

        $response->assertStatus(409);
        $response->assertHeader('X-Wreathe-Location', $this->baseUrl);
    ...
    public function test_validation_errors_are_registered_as_of_default(): void
    {
        Route::middleware([StartSession::class, ExampleMiddleware::class])->get('/', function () {
            $this->assertInstanceOf(\Closure::class, Wreathe::getShared('errors'));
        });
    ...
    {
        Route::middleware([StartSession::class, ExampleMiddleware::class])->get('/', function () {
            $errors = Wreathe::getShared('errors')();
    ...
    public function test_default_validation_errors_can_be_overwritten(): void
    {
        ...
        $response = $this->get('/', ['X-Wreathe' => 'true']);
    ...
    public function test_validation_errors_are_scoped_to_error_bag_header(): void
    {
        ...
        $this->withoutExceptionHandling()->get('/', ['X-Wreathe-Error-Bag' => 'example']);
    }
    ...
    private function prepareMockEndpoint($version = null, $shared = [], $middleware = null): \Illuminate\Routing\Route
    {
        ...
        return Route::middleware(StartSession::class)->get('/', function (Request $request) use ($middleware) {
            return $middleware->handle($request, function ($request) {
                return Wreathe::render('User/Edit', ['user' => ['name' => 'Jonathan']])->toResponse($request);
            });
        });
    }
}
